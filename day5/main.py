...
class update_patient(BaseModel):
    name:Annotated[Optional[str], Field(default=None,description="gve the name of the patient", max_length= 50)]
    city:Annotated[Optional[str],Field(default=None,description="give the name of the city ",max_length=50)]
    age:Annotated[Optional[int],Field(default=None,description="enter age of the patient should not in -",lt=100,gt=0)]
    gender:Annotated[Optional[Literal["male","female","other"]],Field(default=None,description="give the gender of the patient male,female,other")]
    height:Annotated[Optional[float],Field(default=None,description="enter the hight of the patient in meters ",gt=0)]
    weight:Annotated[Optional[float],Field(default=None,description="give the weight of the patient in kg",gt=0)]
...
def updatepatient(patient_id:str , updated_info : update_patient):
    data= load()
    if patient_id not in data:
        raise HTTPException(status_code=403,detail="patient doesnot exist")
    ...
    existing_patientdata["id"]=patient_id

    existing_patientdata=Patient(**existing_patientdata)

    existing_patientdata=existing_patientdata.model_dump(exclude="id")
    data[patient_id]=existing_patientdata
    save_data(data)

    return JSONResponse(status_code=202,content={"message":"upated data successfully"})
